...
import { CuboidCollider, Physics, RapierRigidBody, RigidBody, quat } from "@react-three/rapier";
...
const BlockAxe = ({ position}: {position: [number, number, number ]}): JSX.Element => {
  const obstacle = useRef<RapierRigidBody>(null!)
  const [timeOffset] = useState<number>(() => Math.random() * 2 * Math.PI)
 ...
 <group position={position} >
  ...
  <RigidBody type="kinematicPosition" ref={obstacle} position={position} restitution={0.2} friction={0}>
    <mesh>
